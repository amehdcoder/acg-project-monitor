...
GRANT SELECT, INSERT, DELETE ON public.deleted_account_emails TO authenticated;
GRANT ALL ON public.deleted_account_emails TO service_role;

ALTER TABLE public.deleted_account_emails ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can manage deleted account emails"
...
-- Public-safe helper: lets the signup screen check whether an email was removed.
...
-- Guard the new-user trigger: skip profile/role/notification creation entirely
-- for any email that was permanently removed.
...
BEGIN
  -- Permanently removed accounts must never be recreated.
  IF EXISTS (
    SELECT 1 FROM public.deleted_account_emails

DROP FUNCTION IF EXISTS public.visible_form_submissions(uuid, integer, integer);

CREATE OR REPLACE FUNCTION public.visible_form_submissions(_form_id uuid, _limit integer DEFAULT 1000, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, form_id uuid, user_id uuid, data jsonb, location jsonb, within_geofence boolean, status text, submitted_at timestamp with time zone, synced_at timestamp with time zone, created_at timestamp with time zone, updated_at timestamp with time zone, submission_type text, submission_uuid uuid, client_submitted_at timestamp with time zone, version integer, device_id text, collector_label text)
 LANGUAGE sql
...
AS $function$
  ...
  SELECT
    ...
    fs.device_id, fs.collector_label

/**
 * Self-healing repair for Quiz ⇄ KoboToolbox identity mapping.
 *
 * Configurations saved before participant-identity support existed carry no
 * `nameField` / `nameChoices`, so stored submissions fall back to humanising
 * the raw XML code ("option_2" → "Option 2"). This module re-imports the Kobo
 * schema, persists the detected identity fields, and re-plays the stored raw
 * payloads through the scoring webhook so existing rows pick up real labels.
 */
import { supabase } from "@/integrations/supabase/client";
import { parseKoboForm, type QuizKoboIdentityFields } from "./scoring";
...
/** True when the stored config cannot resolve participant names to labels. */
export function needsIdentityRepair(config?: QuizKoboConfig | null): boolean {
  ...
  return !id.nameField || !(id.nameChoices && id.nameChoices.length > 0);
}
...
export async function repairKoboIdentity(config: QuizKoboConfig): Promise<IdentityRepairResult> {
  ...
  const merged: QuizKoboIdentityFields = {
    ...
    ...Object.fromEntries(Object.entries(parsed.identity).filter(([, v]) => v != null && v !== "")),
  };
  if (!merged.nameField) return { repaired: false, rescored: 0, reason: "no-name-field-on-form" };

  // Drop any identity field that had been persisted as a scorable question.
  const identityNames = new Set(
    [merged.nameField, merged.assessmentField, merged.interventionField].filter(Boolean) as string[],
  );
  const cleanedQuestions = (config.question_config ?? []).filter((q) => !identityNames.has(q.name));

  const { error: upErr } = await supabase
    .from("quiz_kobo_configs")
    .update({
      identity_fields: merged as unknown as any,
      question_config: cleanedQuestions as unknown as any,
    })
    .eq("id", config.id);
  if (upErr) throw upErr;

  const rescored = await rescoreStoredSubmissions(config);
  return { repaired: true, rescored, nameField: merged.nameField };
}

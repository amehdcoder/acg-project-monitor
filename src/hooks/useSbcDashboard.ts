...
async function fetchAll(projectId?: string | null): Promise<SbcRow[]> {
  const all: SbcRow[] = [];
  const PAGE = 1000;
  for (let from = 0; ; from += PAGE) {
    let q = supabase.from("sbc_reports" as any).select(COLUMNS).range(from, from + PAGE - 1);
    if (projectId) q = q.eq("project_id", projectId);
    const { data, error } = await q;
    if (error || !data || data.length === 0) break;
    all.push(...(data as any as SbcRow[]));
...
export const useSbcDashboard = (projectId?: string | null, categoryFilter: SbcCategory | "all" = "all") => {
  ...
  const enriched = useMemo(
    () =>
      rows.map((r) => {
        ...
        const status = (r.status as SbcStatus) || statusFromAchievement(pct);
  ...
  const stats = useMemo(() => {
    ...
    const peopleReached = enriched.reduce(
      (s, r) => s + (Number(r.female_count) || 0) + (Number(r.male_count) || 0),
      0,
    );
    const avgAchievement = total
      ? Math.round(enriched.reduce((s, r) => s + r._pct, 0) / total)
      : 0;
    return { total, onTrack, atRisk, behind, draft, peopleReached, avgAchievement };
  }, [enriched]);

  // Status distribution (donut)
  const statusDistribution = useMemo(() => {
    const counts: Record<SbcStatus, number> = {
    ...
    return (Object.keys(counts) as SbcStatus[]).map((k) => ({
      key: k,

  ...
  return (
    <div className="records-dashboard space-y-2 rounded-lg bg-health-surface p-2 font-report sm:p-3">
      ...
      <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-4">
        <Panel icon={BarChart3} tone="blue" title="Service Uptake by Component"><div className="h-44 p-2"><ChartContainer config={{ value: { label: "Beneficiaries", color: "hsl(var(--records-teal))" } }} className="h-full w-full"><BarChart data={componentData}><XAxis dataKey="name" tick={{ fontSize: 9 }} interval={0} /><YAxis hide domain={[0, maxComponent]} /><Tooltip content={<ChartTooltipContent />} /><Bar dataKey="value" radius={[3, 3, 0, 0]}>{componentData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Bar></BarChart></ChartContainer></div></Panel>

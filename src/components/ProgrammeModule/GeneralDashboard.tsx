...
const Panel = ({ title, subtitle, className, children, action }: {
    ...
    <div className="records-dashboard space-y-2 rounded-lg bg-health-surface p-2 font-report sm:p-3">
      <header className="flex flex-col justify-between gap-3 rounded-md border border-health-blue/10 bg-card px-3 py-3 shadow-soft sm:flex-row sm:items-center">
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-md bg-records-teal/10 text-records-teal"><Users className="h-5 w-5" /></span>
        ...
        <div className="flex items-center gap-3 border-health-blue/15 sm:border-l sm:pl-4">
          <CalendarDays className="h-5 w-5 text-health-blue" />
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
        {kpis.map((item) => <div key={item.label} className={cn("records-kpi rounded-md border bg-card p-3 shadow-soft", `records-kpi-${item.tone}`)}><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md text-primary-foreground"><item.icon className="h-5 w-5" /></span><div><p className="text-[10px] font-bold text-health-ink">{item.label}</p><p className="font-report-display text-2xl font-bold tabular-nums text-health-ink">{item.value.toLocaleString()}</p><p className="text-[9px] font-semibold text-records-green">Current project total</p></div></div></div>)}
      </div>

      <div className="grid gap-2 xl:grid-cols-[2fr_1fr]">
        <Panel title="Beneficiary Journey" subtitle="From enrolment to better health and inclusion" action={<Button variant="ghost" size="sm" onClick={() => onNavigate("journey")}>View report</Button>}>
          <div className="grid grid-cols-2 gap-y-6 px-3 py-5 sm:grid-cols-3 lg:grid-cols-6">
            {journey.map((item, index) => <div key={item.label} className="relative text-center"><div className={cn("mx-auto flex h-11 w-11 items-center justify-center rounded-full border-4 border-card text-primary-foreground shadow-card", `records-stage-${item.tone}`)}><item.icon className="h-5 w-5" /></div>{index < journey.length - 1 && <span className="absolute left-[60%] top-5 hidden h-0.5 w-[80%] bg-records-teal/55 lg:block" />}<p className="mt-2 text-[10px] font-bold text-health-ink">{item.label}</p><p className="font-report-display text-lg font-bold text-health-ink">{item.value.toLocaleString()}</p><p className="text-[9px] text-muted-foreground">Current project</p></div>)}
          </div>
        </Panel>
        <Panel title="Key Alerts & Actions" action={<Button variant="ghost" size="sm" onClick={() => onNavigate("followups")}>View all</Button>}>
      ...
      <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-4">
        <Panel title="Service Uptake by Component"><div className="h-44 p-2"><ChartContainer config={{ value: { label: "Beneficiaries", color: "hsl(var(--records-teal))" } }} className="h-full w-full"><BarChart data={componentData}><XAxis dataKey="name" tick={{ fontSize: 9 }} interval={0} /><YAxis hide domain={[0, maxComponent]} /><Tooltip content={<ChartTooltipContent />} /><Bar dataKey="value" radius={[3, 3, 0, 0]}>{componentData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Bar></BarChart></ChartContainer></div></Panel>
        <Panel title="Beneficiary Outcomes"><RingChart data={outcomes} total={outcomes.reduce((sum, item) => sum + item.value, 0)} label="with outcome" /></Panel>
        <Panel title="Referral Status"><RingChart data={referralData} total={scopedReferrals.length} label="total referrals" /></Panel>
        <Panel title="Facility Performance"><div className="space-y-3 p-3">{facilityData.length ? facilityData.map((facility) => <div key={facility.name}><div className="mb-1 flex justify-between gap-2 text-[10px]"><span className="truncate font-semibold text-health-ink">{facility.name}</span><span>{facility.rate}%</span></div><div className="h-2 overflow-hidden rounded-sm bg-muted"><div className="h-full bg-records-blue" style={{ width: `${facility.rate}%` }} /></div></div>) : <Empty label="No facility-linked records yet" />}</div></Panel>
      </div>

      <div className="grid gap-2 xl:grid-cols-[2fr_1.05fr]">
        <Panel title="Recent Beneficiary Records" subtitle="Latest enrolments and updates across all components">
        ...
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
          <Panel title="Geographic Distribution" action={<Button variant="ghost" size="sm" onClick={() => onNavigate("clusters")}><MapPinned className="h-4 w-4" /> Explore</Button>}><div className="space-y-2 p-3">{lgaData.map(([lga, value], index) => <div key={lga} className="grid grid-cols-[1fr_2fr_auto] items-center gap-2 text-[10px]"><span className="truncate font-semibold text-health-ink">{lga}</span><div className="h-2 overflow-hidden rounded-sm bg-muted"><div className="h-full" style={{ width: `${pct(value, maxLga)}%`, backgroundColor: COLORS[index % COLORS.length] }} /></div><span className="font-bold tabular-nums">{value}</span></div>)}{!lgaData.length && <Empty label="No geographic data recorded" />}</div></Panel>
          <Panel title="Quick Actions"><div className="grid grid-cols-2 gap-2 p-2">{canRegister && <Button variant="outline" className="h-auto justify-start px-2 py-3 text-xs" onClick={onRegister}><Plus className="text-health-blue" /> Add beneficiary</Button>}<Button variant="outline" className="h-auto justify-start px-2 py-3 text-xs" onClick={() => onNavigate("records")}><Search className="text-health-blue" /> Search records</Button><Button variant="outline" className="h-auto justify-start px-2 py-3 text-xs" onClick={() => onNavigate("facility")}><Building2 className="text-health-blue" /> Facilities</Button><Button variant="outline" className="h-auto justify-start px-2 py-3 text-xs" onClick={() => onNavigate("journey")}><FileText className="text-health-blue" /> Generate report</Button></div></Panel>
        </div>
      </div>
    </div>
  );

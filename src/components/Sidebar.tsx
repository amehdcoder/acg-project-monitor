...
const Sidebar = ({ isOpen, onClose, activeTab, onTabChange, profile, role, isAdmin, isOwner, isAdhoc, canAccessPage, minimalAccess, collapsed, onToggleCollapse }: SidebarProps) => {
  ...
  const menuItems = [
    ...
    { id: "location-sharing", label: "My Location Sharing", icon: MapPin, adminOnly: false, showForUsers: true },
    ...
    { id: "integrated-supervisory", label: "Integrated Supervisory", icon: LayoutDashboard, adminOnly: true },
  ...
  const visibleMenuItems = menuItems.filter(item => {
    ...
    if (isRegularUser) {
      ...
      // Quizzes: every project member sees it by default; quiz visibility itself
      // is scoped per project (with a friendly empty state when none exist).
      if (item.id === "quizzes") return true;

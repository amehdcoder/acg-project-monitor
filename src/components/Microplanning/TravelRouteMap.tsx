...
const TravelRouteMap = ({ entries }: TravelRouteMapProps) => {
  ...
  const groupedOrigins = useMemo(() => {
    const flhfs = originLocations.filter(l => l.type === "flhf");
    const comms = originLocations.filter(l => l.type === "community");
    const setts = originLocations.filter(l => l.type === "settlement");
    return { flhfs, comms, setts };
  }, [originLocations]);

  const groupedDestinations = useMemo(() => {
    const flhfs = destinationLocations.filter(l => l.type === "flhf");
    const comms = destinationLocations.filter(l => l.type === "community");
    const setts = destinationLocations.filter(l => l.type === "settlement");
    return { flhfs, comms, setts };

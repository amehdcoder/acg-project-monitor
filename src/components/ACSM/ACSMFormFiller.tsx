...
import {
  ArrowLeft, HelpCircle, Calendar, MapPin, BarChart3, Megaphone, MessageSquare,
...
import {
  ...
  DATA_SOURCES, ACSM_INDICATORS, findIndicator, computeAchievement,
  statusFromAchievement, STATUS_META, achievementColor, formatByUnit, unitLabel,
...
export default function ACSMFormFiller({ projectId, onClose }: Props) {
  ...
  const [category, setCategory] = useState<AcsmCategory>("advocacy");
  ...
  const [actual, setActual] = useState<string>("800");
  const [officer, setOfficer] = useState("");
  const [dataSource, setDataSource] = useState("Beneficiary Register");
  ...
  const [engagement, setEngagement] = useState("Community Outreach");
  const [channel, setChannel] = useState("Community Meetings");
  ...
  const [u18, setU18] = useState<string>("120");
  const [a1835, setA1835] = useState<string>("380");
  const [a35, setA35] = useState<string>("300");
  ...
  const save = async (submission_status: "draft" | "finalized") => {
    ...
    try {
      ...
      await mirrorSpecialForm({
        ...
        formDescription: `${ACSM_CATEGORIES.find((c) => c.value === category)?.label} · ${indDef?.label}`,
      ...
      <div className="sticky top-0 z-20 px-4 sm:px-6 py-4" style={{ background: "rgba(10,22,40,0.9)", backdropFilter: "blur(8px)", borderBottom: `1px solid ${C.borderSoft}` }}>
        <div className="mx-auto flex max-w-7xl items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold leading-tight">ACSM Indicator Reporting Form</h1>
              <p className="text-[13px]" style={{ color: C.sub }}>Report data for Advocacy, Communication & Social Mobilization indicators</p>
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <SectionCard icon={BarChart3} num={1} title="Indicator Details">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                  ...
                  <Sel value={category} onChange={(v) => setCategory(v as AcsmCategory)}>
                    {ACSM_CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </Sel>
                </div>
                <div>
                ...
                <div>
                  <Label required>Stakeholder / Decision-Maker Type</Label>
            ...
            <SectionCard icon={FileText} num={3} title="Narrative & Qualitative Reporting">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <NarrativeBox label="Contribution Story" required value={story} onChange={setStory} max={2000} />
                <NarrativeBox label="Key Challenges" required value={challenges} onChange={setChallenges} max={1000} />
      ...
      <div className="sticky bottom-0 z-20 px-4 sm:px-6 py-3" style={{ background: "rgba(10,22,40,0.95)", backdropFilter: "blur(8px)", borderTop: `1px solid ${C.borderSoft}` }}>
        <div className="mx-auto flex max-w-7xl items-center gap-3">
          <span className="hidden sm:flex items-center gap-1.5 text-[12px]" style={{ color: "#22c55e" }}>
            <span className="h-2 w-2 rounded-full" style={{ background: "#22c55e" }} /> Live Sync: Submissions update the ACSM dashboard in real time.
          </span>

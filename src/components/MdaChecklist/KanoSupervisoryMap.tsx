...
/**
 * Kano-state LGA choropleth for the Integrated MDA Supervisory Checklist
 * dashboard (default project map). Renders the 44 Kano LGAs and populates
 ...
 * and dropping a marker at the exact captured coordinate. Mirrors the Jigawa map.
 */

export interface KanoSubmissionLite {
  id: string;
  lga?: string | null;
...
interface Props {
  submissions: KanoSubmissionLite[];
  formName?: string;
}
...
// GADM truncations used in /nigeria-lga.geojson → readable Kano LGA labels.
const LGA_LABEL: Record<string, string> = {
  dawakink: "Dawakin Kudu",
  dawakint: "Dawakin Tofa",
  nassaraw: "Nasarawa",
  rimingad: "Rimin Gado",
  tundunwada: "Tudun Wada",
  garummallam: "Garum Mallam",
};
...
export default function KanoSupervisoryMap({ submissions, formName }: Props) {
  ...
  // Load + cache Kano LGA polygons once.
  useEffect(() => {
    let cancelled = false;
    ...
    fetch("/nigeria-lga.geojson")
      ...
      .then((d) => {
        if (cancelled) return;
        geoRef.current = (d.features || []).filter((f: any) => norm(f?.properties?.state) === "kano");
        redraw();
      })
      .catch((e) => console.warn("Kano boundaries failed", e));
  ...
  useEffect(() => {
    ...
    L.tileLayer(SATELLITE_TILE_URL, {
      ...
      .on("tileerror", (event) => console.warn("Kano satellite basemap tile failed", event))
      .addTo(map);
    map.setView([12.0022, 8.592], 8);
    mapRef.current = map;
  ...
  function redraw() {
    ...
    // Bold glow underlay so the Kano boundary reads strongly over the basemap.
    ...
    // Animated sweep that replays the supervisory visits.
    if (sweepTimer.current) { clearInterval(sweepTimer.current); sweepTimer.current = null; }
    ...
    <Card className="border-0 shadow-card">
      ...
      <CardContent className="space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <Badge variant="secondary">{lgasTouched} / 44 LGAs covered</Badge>
        </div>
